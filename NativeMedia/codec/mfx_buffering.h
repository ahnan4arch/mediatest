...
public:
    msdkFreeSurfacesPool(WinRTCSDK::Mutex* mutex):
        m_pSurfaces(NULL),
        m_pMutex(mutex) {}
    ...
    inline void AddSurface(msdkFrameSurface* surface) {
        WinRTCSDK::AutoLock lock(*m_pMutex);
...
class msdkUsedSurfacesPool
{
    friend class CBuffering;
public:
    msdkUsedSurfacesPool(WinRTCSDK::Mutex* mutex):
...
class msdkOutputSurfacesPool
{
    friend class CBuffering;
public:
    msdkOutputSurfacesPool(WinRTCSDK::Mutex* mutex):
        m_pSurfacesHead(NULL),
        m_pSurfacesTail(NULL),
        m_SurfacesCount(0),
        m_pMutex(mutex) {}
